...
//         if (Rigidbody2D.velocity.y == 0 && isGrounded) JumpsLeft = MaxJumps;
...
//         //else if the is stuck on the wall, we have to make it fall down
//         else if ((IsWallSliding || !isGrounded) && Rigidbody2D.velocity.y == 0) Rigidbody2D.velocity = new Vector2(Rigidbody2D.velocity.x, -2*WallSlideSpeed);
