    ...
    private void BuildUI() {
        ...
        foreach (UICategory cat in categories) {
            ...
            if (!uiElements.ContainsKey(cat)) {
                ...
                uiElements[cat] = inst;
                elementItemSlot[inst] = inst.transform.Find("Items").transform.Find("Viewport").transform.Find("Content");
            }

            // set visible name
            TMP_Text text = uiElements[cat].GetComponentInChildren<TMP_Text>();
            text.text = cat.name;
